...
#ifdef _OPENMP
#pragma omp parallel for private(ix, iz, v1, v2, diff1, diff2)
#endif
    for (ix=0; ix < nx; ix++) 
    for (iz=0; iz < nz; iz++) 
    {
...
/*< add/subtract seismic sources >*/
{
	...
	if(add){
#ifdef _OPENMP
#pragma omp parallel for private(is, sx, sz)
#endif
		for(is=0;is<ns; is++){
			sx=sxz[is]/nz;
			sz=sxz[is]%nz;
...
/*< record seismogram at time it into a vector length of ng >*/
{
	int ig, gx, gz;
#ifdef _OPENMP
#pragma omp parallel for private(ig, gx, gz)
#endif
	for(ig=0;ig<ng; ig++)
	{
		gx=gxz[ig]/nz;
...
/*< shot/geophone position initialize >*/
{
	int is, sz, sx;
#ifdef _OPENMP
#pragma omp parallel for private(is, sz, sx)
#endif
	for(is=0; is<ns; is++)
	{
		sz=szbeg+is*jsz;
...
/*< scale gradient >*/
{
  ...
  float a;
#ifdef _OPENMP
#pragma omp parallel for private(ix, iz, a)
#endif
  for(ix=1; ix<nx-1; ix++){
    for(iz=1; iz<nz-1; iz++){
	a=vv[ix][iz];
...
#ifdef _OPENMP
#pragma omp parallel for private(i, a) reduction(+:obj)
#endif
  for(i=0; i<ng; i++){
    a=dres[i];
    obj+=a*a;
...
/*< calculate beta >*/
{
  ...
  a=b=c=0;
#ifdef _OPENMP
#pragma omp parallel for private(ix, iz) reduction(+:a,b,c)
#endif
  for(ix=0; ix<nx; ix++){
    for(iz=0; iz<nz; iz++){
...
/*< calculate numerator and denominator of alpha >*/
{
  ...
  float a, b, c;
#ifdef _OPENMP
#pragma omp parallel for private(ig, a, b, c)
#endif
  for(ig=0; ig<ng; ig++){
	c=derr[ig];
...
/*< calculate alpha >*/
{
  ...
  a=b=0;
#ifdef _OPENMP
#pragma omp parallel for private(ig) reduction(+:a, b)
#endif
  for(ig=0; ig<ng; ig++){
    a+=alpha1[ig];
    b+=alpha2[ig];
...
#ifdef _OPENMP
#pragma omp parallel for private(ix, iz, i, s)
#endif
	for(ix=0; ix<nx; ix++)
	for(iz=0; iz<nz; iz++)
	{
...
int main(int argc, char *argv[])
{
	...
	{
		if(verb) {
			gettimeofday(&t1, NULL);// record starting time
		...
		if(verb) {// output important information at each FWI iteration 
			...
			gettimeofday(&t2, NULL);// record ending time 
			sf_warning("iteration %d finished: %f (s)",iter+1, TIME(t1,t2));
		}
	}
